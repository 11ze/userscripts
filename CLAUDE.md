...
- **标题提取**：`extractTitle(el)` 吸收 span→textContent 回退、自挂按钮 emoji（🔍🏆）剥离、`uniqueText` 译名去重（豆瓣「原名称 译名」并列时，按空格分词后从首个与首词同首字的词处截断）
...
测试：`node --test tests/anime_search.test.mjs` 覆盖站点判定（含 query 泄漏回归）、标题提取、按钮渲染与挂载循环；测试通过 `window.__ANIME_SEARCH_TEST__` 条件钩子取用 IIFE 内部函数（浏览器中该钩子永不激活）。
