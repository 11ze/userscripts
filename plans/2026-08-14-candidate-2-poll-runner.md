...
| Q6 | keyed 收敛（原候选 3 主体） | **实施期否决（用户授权代理人裁决，2026-08-14 维持）** | 细读 6 处现场后确认伴随动作位置各异（key 变化时先清复制按钮、insertBefore 插入位、textContent 键），强行统一需回调参数爆炸（过度设计）或改变 tick 内中间步骤（违反 Q4）。唯一完全适配的 `_createCopyNameButton` 单处收敛净复杂度上升。6 处 keyed 现场全部保持原样 |
